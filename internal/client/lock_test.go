...
func TestAcquireInstanceLockAt(t *testing.T) {
	...
	// 4. 父目录不存在时也能成功（嵌套未创建路径）
	nestedPath := filepath.Join(dir, "a", "b", "c", "mesh.lock")
	fourth, err := acquireInstanceLockAt(nestedPath)
	if err != nil {
