...
// UnregisterConn removes the route for ip only if it still points at cc.
//
// 重连竞态：设备断线重连时，新连接可能已 Register 覆盖了同一 IP 的路由，
// 而旧连接的 deferred cleanup 随后才执行。用无条件 Unregister 会把新路由
// 误删，导致新连接收不到包。这里只在当前路由仍是本连接时才删除，避免
// 旧连接清理掉更新的路由。
func (r *Router) UnregisterConn(ip netip.Addr, cc *ClientConn) {
	r.mu.Lock()
	defer r.mu.Unlock()
	if r.routes[ip] == cc {
		delete(r.routes, ip)
